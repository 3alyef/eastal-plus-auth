...
class SearchByEmail {
  public async initialize(
    ...
    try {
      const userData: { userSoul: string; first_name: string } | null =
        await this.findUser(email);
      if (userData) {
        let costumName: costumName | null;
        costumName = await findCostumName(userData.userSoul);
        const userImageDataSearch: searchProfileInt | null =
          await searchProfile(userData.userSoul);

        let userImageData: imageResp | null = {
          lastUpdateIn: undefined,
          userImage: undefined,
        };

        if (
          userImageDataSearch?.lastUpdateIn &&
          userImageDataSearch.userImage
        ) {
          userImageData = {
            lastUpdateIn: userImageDataSearch?.lastUpdateIn,
            userImage: userImageDataSearch?.userImage,
          };
        }
      ...
      console.error(
        "Erro durante a inicialização da pesquisa por email:",
        error
      );
      throw error;
    }
  ...
  private async findUser(
    email: string
  ): Promise<{ userSoul: string; first_name: string } | null> {
    try {
      const user: { soulName: string; first_name: string } | null =
        await userModel.findOne({ email }, "soulName first_name");
