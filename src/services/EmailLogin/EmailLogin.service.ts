...
export class EmailLogin {
    ...
    try {
      ...
      if (email) {
        ...
        if (userData) {
          ...
          const profileImage: searchProfileInt | null = await searchProfile(
            userData.soulName
          );

          if (profileImage) {
            const token: string = new TokenGenerate().TokenGenerator(
              profileImage
            );
  ...
  private async searchEmail(email: string): Promise<userLoginEmail | null> {
    const user: userLoginEmail | null = await userModel.findOne(
      { email: email },
      "email soulName"
    );
    if (user) {
...
export async function searchProfile(
  soulName: string
): Promise<searchProfileInt | null> {
  const image: searchProfileInt | null = await dataUserImageModel.findOne(
    { soulName: soulName },
    "userImage lastUpdateIn"
  );
  if (image) {
    return image;
  } else {
