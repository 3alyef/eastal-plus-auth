...
async function loadDictionary<T>(template: Views, locale: Locale): Promise<T> {
	...
	const jsonPath = pathModule.join(__dirname, `../views-templates/${template}/dictionaries/${locale}.json`);
...
export default async function getDictionary<T>(template: Views, locale: Locale): Promise<T | null> {
  try {
